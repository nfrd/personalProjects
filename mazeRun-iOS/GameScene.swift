...
class GameScene: SKScene, SKPhysicsContactDelegate {
    ...
    func timer(){
        ...
        var levelTimerValue = [11,11,16,11,25,26,9,21, 10, 10, 10]{
            didSet{
    ...
    func spawnArrow(){
        rightArr = SpawnElements().getArrow(445 , yPos: -156, frame: self.frame, imageName: "rightButton.png")
        rightArr.size.height = self.frame.maxY - 644
        rightArr.size.width = self.frame.maxX - 904
        rightArr.alpha = 0.4
        self.addChild(rightArr)
        
        
        leftArr = SpawnElements().getArrow(236, yPos: -156, frame: self.frame, imageName: "leftButton.png")
        leftArr.size.height = self.frame.maxY - 644
        leftArr.size.width = self.frame.maxX - 904
        leftArr.alpha = 0.4
        self.addChild(leftArr)
        

        downArr = SpawnElements().getArrow(341, yPos: -242, frame: self.frame, imageName: "downButton.png")
        downArr.size.height = self.frame.maxY - 670
        downArr.size.width = self.frame.maxX - 885
        downArr.alpha = 0.4
        self.addChild(downArr)
        
        upArr = SpawnElements().getArrow(341, yPos: -70, frame: self.frame, imageName: "upButton.png")
    ...
    let spike1Spot = [[0,0],[222,0],[-222,0],
                      ...
                      [-70,205], [-70,-85], [-70,60],
                      [-128,108], [-18,108], [126, 108], [-110,-100],[-240,-72],
